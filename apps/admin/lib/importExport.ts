...
function markdownToHtml(md: string): string {
  ...
  for (const line of lines) {
    ...
    if (
      /^<(h[1-6]|ul|ol|li|blockquote|pre|hr|img|p|div|video|source)[\s>/]/i.test(
        trimmed,
      )
    ) {
      result.push(line);
...
export async function exportAsZip(
  ...
  turndown.addRule("attachment", {
    filter: (node) =>
      node.nodeName === "A" &&
      (node as HTMLElement).getAttribute("data-attachment") === "true",
    replacement: (_content: string, node: unknown) => {
      const el = node as HTMLElement;
      ...
      return `\n\n<a href="${href}" data-attachment="true" data-key="${key}" data-filename="${fileName}" data-size="${size}" download="${fileName}" class="attachment-card"><span class="attachment-icon">${icon}</span><span class="attachment-meta"><span class="attachment-title">${fileName}</span><span class="attachment-subtitle">点击下载</span></span><span class="attachment-action">下载</span></a>\n\n`;
    },
  });
  ...
  // 5. Extract and download article attachments
  const attachmentRegex = /<a\b[^>]*data-attachment=["']true["'][^>]*>/g;
  const attachments: { tag: string; url: string; fileName: string }[] = [];
  let attachmentMatch: RegExpExecArray | null;
  while ((attachmentMatch = attachmentRegex.exec(markdown)) !== null) {
    const tag = attachmentMatch[0];
    const url = tag.match(/\bhref=["']([^"']+)["']/i)?.[1] || "";
    const fileName =
      tag.match(/\bdata-filename=["']([^"']+)["']/i)?.[1] ||
      getFileNameFromUrl(url) ||
      "attachment";
    attachments.push({ tag, url, fileName });
  }
  ...
  for (const attachment of attachments) {
    try {
      ...
      const fileName = sanitizeAssetFileName(
        attachment.fileName,
        `attachment-${++attachmentIndex}${getMediaExtension(
          attachment.url,
          response.headers.get("content-type"),
        )}`,
      );
      ...
      assetsFolder.file(fileName, blob);
      markdown = markdown.replaceAll(attachment.url, `./assets/${fileName}`);
    } catch (e) {
      console.warn(`Failed to download attachment: ${attachment.url}`, e);
    }
  }
  ...
  const safeName =
    title.replace(/[^a-zA-Z0-9\u4e00-\u9fa5_-]/g, "_").substring(0, 50) ||
    "article";
...
function sanitizeAssetFileName(fileName: string, fallback: string): string {
